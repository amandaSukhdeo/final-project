...
class AuthHelper
{
    ...
    public static function startSession($user) {
        ...
        $_SESSION['id'] = $user['usersId'];           // Add member id to session
        $_SESSION['username'] = $user['usersUid'];     // Add forename to session
        ...
        $userModel = new User();
        $user = $userModel->getUserByID($_SESSION['id']);
        $userModel->updateUserSessionExp(['sessionExpiration' => $toString, 'id' => $user['usersId']]);
    }
